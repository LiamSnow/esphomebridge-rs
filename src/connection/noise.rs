...
use std::{
    error::Error, task::{Context, Poll}, time::Duration
};
...
impl NoiseConnection {
    pub async fn new(ip: &str, noise_psk: String) -> Result<Self, Box<dyn Error>> {
        ...
        let server_name = Self::receive_hello(&mut stream).await?;
        let noise = Self::receive_handshake(&mut stream, noise_handshake).await?;
    ...
    async fn receive_hello(stream: &mut TcpStream) -> Result<String, Box<dyn Error>> {
    ...
    async fn read_frame(stream: &mut TcpStream) -> Result<BytesMut, Box<dyn Error>> {
...
impl Connection for NoiseConnection {
    ...
    async fn receive_message_raw(&mut self) -> Result<(MessageType, BytesMut), Box<dyn Error>> {
        let frame = Self::read_frame(&mut self.stream).await?;
    ...
    async fn receive_message<T: prost::Message + Default>(
        &mut self,
        expected_msg_type: MessageType,
    ) -> Result<T, Box<dyn Error>> {
        let (msg_type, mut msg) = self.receive_message_raw().await?;
    ...
    async fn transaction<T: prost::Message + Default>(
        &mut self,
        req: impl prost::Message,
        req_type: MessageType,
        res_type: MessageType,
    ) -> Result<T, Box<dyn Error>> {
        self.send_message(req, req_type).await?;
        Ok(self.receive_message(res_type).await?)
    }

    async fn buffer_empty(&mut self) -> bool {
        let waker = futures::task::noop_waker();
        let mut cx = Context::from_waker(&waker);
        matches!(self.stream.poll_read_ready(&mut cx), Poll::Ready(Ok(())))
    }

    async fn disconnect(&mut self) -> Result<(), std::io::Error> {
